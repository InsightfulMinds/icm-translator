...
No model runs at conversion time. `checker/convert.mjs` is a plain script that can only copy bytes
out of the input. The same folder also works as a Claude project (the brief this was built for asks
...
`checker/convert.mjs` is extractive. Every string it emits comes from one function that slices the
input buffer between two offsets, so there is no code path that can put a character into a card that
was not in the transcript.
...
The verifier runs thirteen checks, from schema validity through byte-identical quotes, word
boundaries, step ordering, the relationship between a name and the phrase that introduced it, and a
coverage recomputation that flags any run of real input no span touches. All thirteen are listed with
what each one does and does not prove in
[the walkthrough](docs/WALKTHROUGH.md#what-the-verifier-actually-checks).
...
each one fires exactly its own error code and nothing else. A verifier that rejected everything would
pass a weaker test.
...
trips exactly its own alarm and no other.*
...
filename before a byte of it was read. The unit tests never saw it, because every test used the
repo's own inputs. A walk through a clean clone caught it. That check was removed in `410d01a`, its
fixture (`neg-23`) was retired, and the selftest now converts and verifies two transcripts the
converter has never seen. A second review pass found two more holes, a duplicate key hidden behind a
`title` escape and one claim listed three times at the same span, both closed in `7c84833` with
fixtures `neg-26` and `neg-27`. The count now stands at 138.
