...
# 156 assertions: 26 staged inventions, shape drift, unseen input, the CLI (exit 0 / 1)
...
Save your transcript as a plain UTF-8 `.txt` file inside `inputs/`. The name does not matter. The
card cites the file and the verifier re-reads sources only from `inputs/` (and `fixtures/`), so
`convert.mjs` refuses a file anywhere else and says so. You can type the path from anywhere,
...
(`88b7d59`). Work added after the deadline is marked as such on this page.
...
the converter refuses up front a file the verifier would later reject (anything outside `inputs/`
and `fixtures/`), every command answers `--help`, and eleven command-line assertions, one of them
staging a symlinked checkout, bring the count to 156.
...
.github/       workflows/check.yml runs the four commands above on Ubuntu and macOS for every push
```
