...
[2, the contract](#2--the-contract-is-written-down-in-reference) ·
[3, every fact traces](#3--every-fact-traces-to-the-input) ·
[what the verifier checks](#what-the-verifier-actually-checks) ·
[4, staged inventions](#4--twenty-six-staged-inventions-six-walked-through-in-detail-below) ·
[what it does not do](#what-it-does-not-do) · [limits](#limits-stated-plainly) ·
[layout](#layout) · [room to expand](#room-to-expand)
...
node checker/selftest.mjs       # 156 assertions, 26 staged inventions among them (exit 0 / 1)
```
...
the folder does, because the card cites the file and the verifier re-reads sources only from
`inputs/` (and `fixtures/`). `convert.mjs` refuses a file anywhere else with a message saying so,
and accepts the path typed from anywhere, absolute or relative to where you are. The run below
used a 256-byte file saved as `inputs/quickstart-demo.txt`.
...
A gate nobody has seen fail is not a gate. The full run is 156 assertions.
...
...156 individual "pass" lines in ten groups, listed below...
156 passed, 0 failed.
exit=0
```
...
`selftest.mjs`'s 156 assertions are grouped ten ways. The group headers below name what each group
...
(`fixtures/e2e-03-suffixed-figures.txt`), and after the deadline to 156 with eleven command-line
assertions: `--help` and bad usage on each command, a transcript outside the repo, a repo file
outside `inputs/` and `fixtures/` (which `convert.mjs` used to turn into a card that could never
verify), and an absolute path through a symlinked checkout, which it used to report as not found. Most groups above iterate a
...
cards/         the 4 outputs
assets/        social-preview.png, for the repo's link preview (takes effect only once uploaded under Settings > Social preview)
audits/        generated by shape-diff.mjs, not committed; your run writes it
.github/       workflows/check.yml runs the four commands on Ubuntu and macOS for every push
```
